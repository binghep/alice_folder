...
foreach ($results as $key => $value) {
    array_push($all_possible_skus,$value['sku']);
}

/*
// var_dump($all_possible_skus);
$eligible_products=array();//has at least one non-empty attribute
foreach ($all_possible_skus as $sku) {
    // var_dump($sku);
...
// echo '</pre>';
// return;
	$w_id=$product->get_worker_id();
	$worker_name=$product->get_worker_name();
	//----------for myself.php--------------
	if ($w_id!==$worker_id){continue;}
	//----------for myself.php--------------
	// var_dump($worker_id);
...
$pdf->SetTitle('Price Research Report for External products - Myself');
