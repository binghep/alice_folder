...
">
  		<a style="height:19px;" href="<?php echo $product_manager_url;?>standalone.index.php">Back to Home</a>
  	</div>
	...
	<form  style="border:none;" id="form3" name="form3" class="wufoo topLabel page1" accept-charset="UTF-8" autocomplete="off" action="standalone.search_product.employee_version.php" method="get" novalidate="" >
  
		<header id="header" class="info">
			<h2>Edit Product / 编辑产品</h2>
		</header>

		<label>Find product I uploaded with Name keyword:</label>
...
if (!empty($_GET['submit']) && !empty($_GET['search_keywords'])){
	...
	$query="select worker_id, products.sku as total_num_result from products 
inner join standalone_product_eav_attribute_values
ON products.sku=standalone_product_eav_attribute_values.sku
where products.name LIKE '%{$search_keywords}%' and worker_id='{$worker_id}'
...
// var_dump($query);
	...
	if (is_null($result)){
		...
		echo 'You did not upload any products with this keyword in its name. Please search another keyword that might be in product name. <br>';
		echo '</div>';
		exit;
	}
	$total_num_result=0;
	...
	//---get all skus that belong to current worker and has keyword.---
	$query="select worker_id, products.sku from products 
inner join standalone_product_eav_attribute_values
ON products.sku=standalone_product_eav_attribute_values.sku
where products.name LIKE '%{$search_keywords}%' and worker_id='{$worker_id}'
group by products.sku 
LIMIT $start_row_index,$num_product_per_page;";
/*e.g. 
		select worker_id, products.sku from products 
		inner join standalone_product_eav_attribute_values
		ON products.sku=standalone_product_eav_attribute_values.sku
		where products.name LIKE '%bento%' and worker_id=12
		group by products.sku LIMIT 0,2;
*/
   ...
   <!--  <thead>
	...
	foreach ($eligible_products as $object) {
		...
		echo '<td class="td_elements">';
			echo '<div class="img_div"><a href="standalone.edit.php?id='.$object->id.'&callback_url='.urlencode($product_manager_url."standalone.search_product.employee_version.php?search_keywords=".urlencode($search_keywords)."&submit=Submit").'"><img style="margin: auto;max-width: 150px;max-height: 150px;" src="'.$product_manager_url.$object->image.'" ></a></div>';
