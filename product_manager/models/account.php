...
class Account {
	...
	public static function find_acc_id ( $login ) {
		...
		if ($conn->connect_error) {
			...
			$sql_query = 'SELECT acc_id FROM justin_workers WHERE acc_login=\''.$login.'\' LIMIT 1;';
	...
	public function load_account ( $acc_id ) {
		...
		// Check connection
			...
			$sql_query = 'SELECT * FROM justin_workers WHERE acc_id='.(int)$acc_id.' LIMIT 1;';
			$result = $conn->query($sql_query);
			if ($row = $result->fetch_assoc()) {
			$conn->close();
	...
	public function delete () {
		...
		if ($conn->connect_error) {
			...
			$sql_query = 'DELETE FROM justin_workers WHERE acc_id='.(int)$this->acc_id.' LIMIT 1;';
			$result = $conn->query($sql_query);
			if ($result) {
				$conn->close();
				return $this->acc_id;
	...
	public function update () {
		...
		if ($conn->connect_error) {
			...
			$sql_query = 'SELECT acc_id FROM justin_workers WHERE (acc_login=\''.$this->acc_login.'\')AND(acc_id<>'.(int)$this->acc_id.') LIMIT 1;';
			$result = $conn->query($sql_query);
			...
			$result->close();
			//update
			$sql_query = 'UPDATE justin_workers SET acc_login=\''.$this->acc_login.'\', acc_name=\''.$this->acc_name.'\', acc_password=\''.sha1($this->acc_password).'\' WHERE acc_id='.(int)$this->acc_id.' 
			LIMIT 1;';
			$result = $conn->query($sql_query);
			if ($result) {
				$conn->close();
	...
	public function add_new_account () {
		...
		if ($conn->connect_error) {
			...
			//first check to see if LOGIN is not used by another user:
			$sql_query = 'SELECT acc_id FROM justin_workers WHERE (acc_login=\''.$this->acc_login.'\') LIMIT 1;';
			$result = $conn->query($sql_query);
			if ($row = $result->fetch_assoc()) {
				$conn->close();
			...
			$result->close();
			//insert
			$sql_query = 'INSERT INTO justin_workers (acc_login, acc_name, acc_password, is_admin) VALUES (\''.$this->acc_login.'\', \''.$this->acc_name.'\', \''.sha1($this->acc_password).'\',0);';
			$result = $conn->query($sql_query); 
			if ($result) {
				$conn->close();
				return 1;
